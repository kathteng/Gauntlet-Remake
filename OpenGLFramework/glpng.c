...
static void Build2DMipmaps(GLint components, GLint width, GLint height, GLenum format, const unsigned char *data, int filter, unsigned int target)
{
	int level = 0;
	unsigned char *d = (unsigned char *) malloc((width/2)*(height/2)*components+4);
	const unsigned char *last = data;

	glTexImage2D(target, level, components, width, height, 0, format, GL_UNSIGNED_BYTE, data);
	level++;
	...
	while (HalfSize(components, width, height, last, d, filter)) {
		...
		glTexImage2D(target, level, components, width, height, 0, format, GL_UNSIGNED_BYTE, d);
		level++;
		last = d;
	}
...
int APIENTRY pngLoad(const char *filename, int mipmap, int trans, pngInfo *pinfo, unsigned int target)
{
	int result;
	FILE *fp = fopen(filename, "rb");
	if (fp == NULL) return 0;

	result = pngLoadF(fp, mipmap, trans, pinfo, target);
...
int APIENTRY pngLoadF(FILE *fp, int mipmap, int trans, pngInfo *pinfo , unsigned int target)
{
	GLint pack, unpack;
	unsigned char header[8];
	png_structp png;
...
*/
	...
	{ /* OpenGL stuff */
		...
		if (PalettedTextures && mipmap >= 0 && trans == PNG_SOLID && color == PNG_COLOR_TYPE_PALETTE) {
			...
			glColorTableEXT(target, GL_RGB8, cols, GL_RGB, GL_UNSIGNED_BYTE, pal);
			glTexImage2D(target, mipmap, intf, width, height, 0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, data);
		}
		else
		#endif
		if (trans == PNG_SOLID || trans == PNG_ALPHA || color == PNG_COLOR_TYPE_RGB_ALPHA || color == PNG_COLOR_TYPE_GRAY_ALPHA) {
			...
			if (mipmap == PNG_BUILDMIPMAPS)
				Build2DMipmaps(glcomponent, width, height, glformat, data, 1, target);
			else if (mipmap == PNG_SIMPLEMIPMAPS)
				Build2DMipmaps(glcomponent, width, height, glformat, data, 0, target);
			else
				glTexImage2D(target, mipmap, glcomponent, width, height, 0, glformat, GL_UNSIGNED_BYTE, data);
		}
		else {
			png_bytep p, endp, q;
			int r, g, b, a;
			...
			if (mipmap == PNG_BUILDMIPMAPS)
				Build2DMipmaps(4, width, height, GL_RGBA, data2, 1, target);
			else if (mipmap == PNG_SIMPLEMIPMAPS)
				Build2DMipmaps(4, width, height, GL_RGBA, data2, 0, target);
			else
				glTexImage2D(target, mipmap, 4, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data2);
...
/*! \brief Return OpenGL texture bind target that corresponds with given image target

    All faces of a cubemap have different enumerations that glTexImage2D uses but
    they all share a single target that glBindTexture uses.

*/
static unsigned int BindTargetFromImageTarget( unsigned int uImageTarget )
{
    if( ( uImageTarget >= GL_TEXTURE_CUBE_MAP_POSITIVE_X ) && ( uImageTarget <= GL_TEXTURE_CUBE_MAP_NEGATIVE_Z ) )
    {
...
static unsigned int SetParams(int wrapst, int magfilter, int minfilter, unsigned int target)
{
	unsigned int id;
    target = BindTargetFromImageTarget( target ) ;

	glGenTextures(1, &id);
	glBindTexture(target, id);

	glTexParameteri(target, GL_TEXTURE_WRAP_S, wrapst);
	glTexParameteri(target, GL_TEXTURE_WRAP_T, wrapst);
...
unsigned int APIENTRY pngBind(const char *filename, int mipmap, int trans, pngInfo *info, int wrapst, int minfilter, int magfilter, unsigned int target)
{
	unsigned int id = SetParams(wrapst, magfilter, minfilter, target);

	if (id != 0 && pngLoad(filename, mipmap, trans, info, target))
		return id;
	return 0;
}
...
unsigned int APIENTRY pngBindF(FILE *file, int mipmap, int trans, pngInfo *info, int wrapst, int minfilter, int magfilter, unsigned int target)
{
	unsigned int id = SetParams(wrapst, magfilter, minfilter, target);

	if (id != 0 && pngLoadF(file, mipmap, trans, info, target))
		return id;
	return 0;
}
